...
const required=['index.html','app.css','log.css','soc.css','ops.css','v05.css','core.js','log-engine.js','soc-engine.js','ops-engine.js','security-runtime.js','app.js','log-ui.js','soc-ui.js','ops-ui.js','ui-hardening.js','soc-v05-ui.js','favicon.svg','manifest.webmanifest','sw.js'];
...
for(const ref of ['app.css','log.css','soc.css','ops.css','v05.css','core.js','log-engine.js','soc-engine.js','ops-engine.js','security-runtime.js','app.js','log-ui.js','soc-ui.js','ops-ui.js','ui-hardening.js'])if(!html.includes(ref))bad('index missing ref',ref);
...
const sw=fs.readFileSync(new URL('sw.js',root),'utf8');for(const ref of required.filter(x=>x!=='sw.js'))if(!sw.includes(`./${ref}`))bad('service worker missing asset',ref);if(!/const CACHE='dbm-v7'/.test(sw))bad('unexpected PWA cache version');
...
const netlify=fs.readFileSync(new URL('../netlify.toml',import.meta.url),'utf8');if(!netlify.includes('Content-Security-Policy'))bad('Netlify CSP missing');
